...
func TestWrapError(t *testing.T) {
	var err error
	err = WrapError(err, map[string]interface{}{"author": "henrylee2cn"})
	...
	err = WrapError(err, map[string]interface{}{"author": []string{"henrylee2cn"}})
	assert.EqualError(t, err, `author=[henrylee2cn], error=error text2`)

	err = errors.New("error text3")
	err = WrapError(err, map[string]interface{}{"author": struct {
		Name string
		X    int
	}{Name: "henrylee2cn"}})
	assert.EqualError(t, err, `author={Name:henrylee2cn X:0}, error=error text3`)

	err = WrapError("error text4", map[string]interface{}{"author": "henrylee2cn"})
